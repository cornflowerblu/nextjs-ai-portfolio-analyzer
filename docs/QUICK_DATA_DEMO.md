...
- `/app/api/analyses/route.ts` exposes the cached ISR endpoint (`revalidate = 60`, KV-backed). It returns KV hit/miss counts plus `mode` metadata when env vars are missing.
- `/app/api/analyses/live/route.ts` is `force-dynamic` on the Edge runtime to highlight latency and runtime differences.
- `lib/lab/analyses.ts` wraps Vercel Postgres + KV access with fallbacks, cache invalidation, and stats tracking.
- `app/lab/cache/page.tsx` wires a Server Action that inserts into Postgres, clears the KV list, and revalidates the cached route.
